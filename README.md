...
~~4.Merchant Admin - this user works for a merchant, and has additional capabilities than regular employees, such as changing merchant info.~~ (we are going to combine these merchant roles into one for the rest of the project. If you've already distinguished between merchant employee's and merchant admin's, you can keep your code as is, but just make all merchants employee's from now on)

5. Admin User - a registered user who has "superuser" access to all areas of the application; user is logged in to perform their work
...
## Timeframe for Part 2
The following is an anticipated timeline of how these stories should be completed in order to be finished by 1/9/20
...
Admin users can access a namespaced route of "/admin/users" to see an index page  of all non-merchant/non-admin users, and from there see each user. This will allow the admin to perform every action on a user's account that the user themselves can perform. This funcitonality is taken care of in User Stories 53 & 54. ~~Admin users can also "upgrade" a user account to become a merchant account.~~
...
### Registered Users and Merchants only
~~Merchants and~~ (we are not going to restrict merchants to order items since we've removed the functionality for admin to upgrade users and this is no longer something we need to take into account for this project). However, Admin users still cannot order items. This should already be taken care of through the navigation restrictions you set in User Stories 5 & 9. 
...
As a registered user or merchant
...
I am taken to my orders page 
I see a flash message telling me my order was created
I see my new order listed on my orders page
...
- Admins can fulfill items on order on behalf of a merchant (this is in an extension story)
...
I see a button or link to cancel the order ~only~if~the~order~is~still~pending~ as long as the order has not shipped
...
As an admin 
When I visit the merchant index page ("/admin/merchants")
...
| --- | --- | --- | --- | --- | --- |
| **4: Exceptional**  | All User Stories 100% complete including all sad paths and edge cases, and some extension work completed | Students implement strategies not discussed in class to effectively organize code and adhere to MVC. | Highly effective and efficient use of ActiveRecord beyond what we've taught in class. Even `.each` calls will not cause additional database lookups. | Very clear Test Driven Development. Test files are extremely well organized and nested. Students utilize `before :each` blocks. 100% coverage for features and models | Final project has a well written README with pictures, schema design, code snippets, contributors names linked to their github profile, heroku link, and implementation instructions. |
